...
function initRender (vm) {
  ...
  vm._c = (a, b, c, d) => createElement(vm, a, b, c, d, false); // template转render会调用_c
  // normalization is always applied for the public version, used in
  // user-written render functions.
  // 对手写 render 函数进行渲染的方法
  vm.$createElement = (a, b, c, d) => createElement(vm, a, b, c, d, true); // 相当于h函数，虚拟DOM转为真实DOM
  // h函数
...
function renderMixin (Vue) {
  ...
  Vue.prototype._render = function () {
    const vm = this;
    const { render, _parentVnode } = vm.$options; // 渲染函数：用户定义的render或者是模板渲染的render
...
function initProps (vm, propsOptions) {
  ...
  for (const key in propsOptions) { // 遍历传进来的props对象
    ...
    /* istanbul ignore else */
    {
      ...
      defineReactive$$1(props, key, value, () => { // 注入到vm._props对象中并设置为响应式
...
function initComputed (vm, computed) {
  ...
  for (const key in computed) {
    ...
    const getter = typeof userDef === 'function' ? userDef : userDef.get; // 不是函数就调用它自己响应式的那个get方法
    if (getter == null) {
      warn(
...
function initGlobalAPI (Vue) {
  ...
  {
    configDef.set = () => { // 给config赋值的时候会出现警告
      warn(
